...
function AdminShell({ children, active = 'dashboard', title, subtitle, topRight, noPad }) {
  ...
  return (
    <div style={{ width: '100vw', minHeight: '100vh', height: '100vh', display: 'flex', fontFamily: 'Poppins', background: '#F5F5F5', overflow: 'hidden' }}>
      {/* Sidebar */}
      <div style={{ width: 240, background: NAVY, color: 'white', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        <div style={{ padding: '20px 20px 16px' }}>
          <div onClick={() => navTo('dashboard-lgu.html')} style={{ cursor: 'pointer', display: 'inline-block' }}>
        ...
        <div style={{ flex: 1, padding: '10px 12px' }}>
          {nav.map(([id, icon, label]) => (
            <div key={id} onClick={() => navTo(ADMIN_WEB_ROUTES[id])} style={{
